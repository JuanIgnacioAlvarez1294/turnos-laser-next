...
import { getServiciosMap, getTurnos, updateTurno, deleteTurno } from "@/services/turnos.service";
  ...
  return (
    <div className="max-h-[400px] overflow-y-auto rounded-xl shadow border border-pink-300 bg-white mt-8">
      <table className="min-w-full bg-white border border-rosa-pastel shadow-xl rounded-xl">
        ...
        <tbody>
          {turnos.map((turno) => (
            <tr key={turno.turnoId} className="text-center hover:bg-rosa-fondo transition">
              <td className="py-2 px-4 border-b">{decodeURIComponent(turno.turnoId)}</td>
              ...
              <td className="py-2 px-4 border-b flex justify-center gap-3">
                ...
                <button
                  onClick={() => (window.location.href = `/admin/turnos/${turno.turnoId}`)}
